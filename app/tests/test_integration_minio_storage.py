...
class TestIntegrationMinIOStorage:
    """Integration tests for MinIO storage with stream initialization and video processing."""
    ...
    def test_stream_recording_and_video_processing_multiple_durations_integration(
        ...
        stream_id = f"integration-test-stream-minio-{video_duration}s"
        stream_path = f"integration-test-path-minio-{video_duration}s"

        log.info(f"Using stream ID: {stream_id} with path: {stream_path}")

        # 0. Check if stream already exists and remove it if it does
        existing_health = client_with_minio_storage.get(
        ...
        if existing_health.status_code == 200:
            ...
            delete_response = client_with_minio_storage.delete(
        ...
        while time.time() - start_time < max_wait_time:
            health_response = client_with_minio_storage.get(
                f"/api/streams/{stream_id}/health"
            )
        ...
        process_response = client_with_minio_storage.post(
        ...
        while time.time() < deadline:
            status_response = client_with_minio_storage.get(
        ...
        output_uri = final_result.get("result_video_uri")
        assert output_uri is not None, "No output video URI returned"
        assert output_uri.startswith(
            "http"
        ), f"Expected presigned URL, got: {output_uri}"

        # Download the video from presigned URL to check duration
        with tempfile.NamedTemporaryFile(suffix=".mp4", delete=False) as temp_file:
            try:
                urlretrieve(output_uri, temp_file.name)
                actual_video_duration = get_video_duration(temp_file.name)
                ...
                log.info(
                    f"Video processing completed successfully. Output URI: {output_uri}, Duration: {actual_video_duration}s"
                )
            finally:
        ...
        delete_response = client_with_minio_storage.delete(f"/api/streams/{stream_id}")
        assert (
    ...
    def test_stream_recording_and_video_processing_integration(
        ...
        stream_id = "integration-test-stream-minio"
        stream_path = "integration-test-path-minio"
        ...
        with tempfile.NamedTemporaryFile(suffix=".mp4", delete=False) as temp_file:
            try:
                urlretrieve(output_uri, temp_file.name)
                video_duration = get_video_duration(temp_file.name)
                ...
                log.info(
                    f"Video processing completed successfully. Output URI: {output_uri}, Duration: {video_duration}s"
                )
            finally:
                # Cleanup: Remove temporary downloaded file
                try:
    ...
    def test_stream_recording_and_future_video_processing_integration(
        ...
        stream_id = "integration-test-stream-minio-future"
        stream_path = "integration-test-path-minio-future"
        ...
        with tempfile.NamedTemporaryFile(suffix=".mp4", delete=False) as temp_file:
            try:
                ...
                log.info(
                    ...
                    f"Output URI: {output_uri}, Duration: {actual_video_duration}s"
                )
            finally:
                # Cleanup: Remove temporary downloaded file
                try:
